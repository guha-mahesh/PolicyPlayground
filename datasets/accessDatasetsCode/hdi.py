...
def download_hdi_data(url="https://hdr.undp.org/data-center/documentation-and-downloads"):
    ...
    try:
        ...
        try:
            df_alt = pd.read_excel(excel_content, sheet_name=0, skiprows=4, header=0)
            df_alt = df_alt.dropna(how='all').dropna(axis=1, how='all')
            return clean_hdi_dataframe(df_alt)
...
def display_data_summary(df):
    """Display a summary of the HDI data"""
    if df is None:
        print("No data to display")
        return
    
    print("\n" + "="*60)
    print("HDI TRENDS DATA SUMMARY")
    print("="*60)
    ...
    # Show some basic statistics for HDI values
    hdi_columns = [col for col in df.columns if col.isdigit() and len(col) == 4]
    if hdi_columns:
        print(f"\nHDI Value Statistics for year columns:")
        print(df[hdi_columns].describe())
...
if __name__ == "__main__":
    ...
    if hdi_df is not None:
        final_df = display_data_summary(hdi_df)
