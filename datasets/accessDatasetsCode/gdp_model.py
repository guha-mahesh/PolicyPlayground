...
# Global variables to match original structure
feature_means = {}
feature_stds = {}
country_features = []
...
def fetch_worldbank_data(indicator, start_year=2010, end_year=2023):
    """Fetch data from World Bank API and format to match original API structure"""
    ...
    try:
        ...
        # Format to match original API structure
        formatted_data = []
        for item in data[1]:
            if item['value'] is not None:
                formatted_data.append({
                    'country': country_code_to_name.get(item['countryiso3code'], item['countryiso3code']),
                    'mos': item['date'],
                    'vals': item['value']
                })
...
def train_func():
    ...
    def prepare_training_data():
        # Fetch data from World Bank API instead of custom API
    ...
    # Store coefficients globally instead of API (since we don't have the original API)
    global stored_coefficients
    stored_coefficients = coefficients
...
def predict(user_features, country, current_year=2024, coefficients=None):
    ...
    if coefficients is None:
        # Use stored coefficients instead of API call
        global stored_coefficients
...
# Global variable to store coefficients (replaces API storage)
stored_coefficients = None


def main():
    """Main function to demonstrate the model"""
    print("GDP Prediction Model - Exact Original Structure")
    print("=" * 50)
    ...
    # Sample predictions using the exact same interface as original
    test_cases = [
        ([8.5, 5.0, 3.5], "USA", 2024),
        ([9.0, 4.8, 1.2], "DEU", 2024),
        ([7.8, 3.2, 1.0], "JPN", 2024),
        ([7.5, 5.5, 2.1], "GBR", 2024),
        ([9.2, 5.3, 2.0], "FRA", 2024),
        ([6.8, 5.2, 1.3], "CAN", 2024)
    ]
    
    for features, country, year in test_cases:
        try:
            prediction = predict(features, country, year)
            print(f"\nCountry: {country}")
            print(f"Health: {features[0]}%, Education: {features[1]}%, Military: {features[2]}%")
            print(f"Predicted GDP per capita: ${prediction:,.2f}")
        except Exception as e:
            print(f"Error predicting for {country}: {e}")
    
    print("\n" + "=" * 50)
    print("Model training and prediction completed!")
    print(f"Global variables populated:")
    print(f"- feature_means: {len(feature_means)} features")
    print(f"- feature_stds: {len(feature_stds)} features") 
    print(f"- country_features: {len(country_features)} countries")
    print(f"- log_transformed_features: {len(log_transformed_features)} features")
