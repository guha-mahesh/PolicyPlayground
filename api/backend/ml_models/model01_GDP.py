...
def prepare_training_data():
    ...
    df = df.merge(govt_health.rename(columns={'value': 'Health_spending'}), on=['Country', 'date'], how='outer')
    df = df.merge(govt_education.rename(columns={'value': 'Education_spending'}), on=['Country', 'date'], how='outer')
    df = df.merge(military_spending.rename(columns={'value': 'Military Spending'}), on=['Country', 'date'], how='inner')
...
def train_gdp_model():
    ...
    lagged_features = ['Health_spending_lag1', 'Education_spending_lag1', 'Military Spending_lag1']
    ...
    model_data, gdp_feature_means, gdp_feature_stds = normalize_features(model_data, fit=True)
...
def predict_gdp(user_features, country, current_year=2024, 
    ...
    if coefficients is None:
        raise ValueError("Model coefficients not available. Train model first.")
        
    if country not in country_code_to_name.values():
        raise ValueError(f"Country must be one of: {list(country_code_to_name.values())}")
...
if __name__ == "__main__":
    try:
        predicted_gdp = predict_gdp(
            user_features=[10.9, 3.6, 1.0],  # health%, education%, military% of GDP
            country="Japan",
