...
with col_left:
    ...
    st.write(f"• Federal Funds Rate: {fed_funds_input or fed_funds_slider}%")
    st.write(f"• Money Supply: {policy_config['currency']}{money_supply_input or money_supply_slider:,}B")
    st.write(f"• Reserve Requirement: {reserve_ratio_input or reserve_ratio_slider}%")
...
with col_right:
    ...
    st.write(f"• Infrastructure Spending: {infrastructure_input or infrastructure_slider}% of GDP")
    st.write(f"• Debt-to-GDP Ratio: {debt_gdp_input or debt_gdp_slider}%")
    st.write(f"• Corporate Tax Rate: {corp_tax_input or corp_tax_slider}%")
tables = [
    "YuantoUSD",
    "AUDtoUSD",
    "JPYtoUSD",
